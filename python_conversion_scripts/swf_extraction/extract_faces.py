...
# Run: uv run python3 python_conversion_scripts/swf_extraction/extract_faces.py
import subprocess
import sys
import tempfile
...
from swf_xml_lib import (  # noqa: E402
    WEB_SWF_XML, make_char_bounds, parse_swf_xml, snapshot_timeline,
)
...
FACE_FRAMES = {1: "veradux", 2: "roald", 3: "felicity", 4: "wolfgang", 5: "amber", 6: "sonny"}
...
def main():
    ...
    subprocess.run(
        [str(FFDEC), "-zoom", str(ZOOM), "-format", "shape:png", "-selectid",
         ",".join(str(c) for c in sorted(needed)), "-export", "shape",
    ...
    for frame, name in FACE_FRAMES.items():
        ...
        for depth in sorted(snaps.get(frame, {})):
            ...
            paste_char(canvas, entry["cid"], entry.get("mat", (1, 0, 0, 1, 0, 0)), origin)
