...
#
# Run: uv run python3 python_conversion_scripts/swf_extraction/extract_doll_art.py
import subprocess
import sys
import tempfile
...
def build_renderer(xml_path: Path, swf_path: Path):
    ...
    def export_shapes(needed):
        ...
        for i in range(0, len(missing), 400):
            subprocess.run(
                [str(FFDEC), "-zoom", str(ZOOM), "-format", "shape:png",
                 "-selectid", ",".join(str(c) for c in missing[i:i + 400]),
                 "-export", "shape", str(shape_dir), str(swf_path)],
...
def main():
    ...
    print(f"re-rendered: {done}, skipped (placeholders): {len(skipped)}", file=sys.stderr)
    if skipped:
