...
#
# Run: uv run python3 python_conversion_scripts/swf_extraction/extract_item_looks.py
import json
import re
import sys
...
from swf_models import ACTION_SCRIPT, CONVERTED_JSON  # noqa: E402

REPO = Path(__file__).resolve().parent.parent.parent
SRC = ACTION_SCRIPT / "frame_42" / "DoAction_16.as"
ITEMS_DIR = REPO / "resources" / "items"
OUT = CONVERTED_JSON / "item_looks.json"

CREATE_RE = re.compile(r'createNewItemKrin\("(.*?)",(-?\d+),')
LOOKS_RE = re.compile(r'(?:gghhjjuu|_root\["KRINITEM" \+ itemKrinIDnow\])\.looks = "([^"]*)"')
