...
#
# Run: uv run python3 python_conversion_scripts/swf_extraction/extract_model1_animations.py
import json
import re
import sys
...
from swf_xml_lib import WEB_SWF_XML, snapshot_timeline

REPO = Path(__file__).resolve().parent.parent.parent
OUT = REPO / "resources" / "sprites" / "model1_animations.json"
MODEL_SPRITE = 166
TOTAL_FRAMES = 371
FPS = 30.0

PART_NAMES = {
    "head", "chest", "arm1", "arm2", "hand1", "hand2", "leg1", "leg2",
    "leg3", "leg4", "foot1", "foot2", "weapon1", "weapon2", "shoulder",
}


def main():
    xml = WEB_SWF_XML.read_text()
    snaps, labels = snapshot_timeline(xml, MODEL_SPRITE, set(range(1, TOTAL_FRAMES + 1)))
    ...
    for frame in range(1, frame_count + 1):
        ...
        for entry in snap.values():
            ...
            pose[part] = [
                ...
                round(tx / 20.0, 2), round(ty / 20.0, 2),
            ]
        frames.append(pose)
