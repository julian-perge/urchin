...
```sh
uv run python3 conversion_scripts/swf_extraction/export_swf_sources.py
```
...
| `action_script_curated/` | Hand-picked, commented excerpts used by the original data ports (previously `conversion_scripts/action_script_files/`). |
...
Analysis and extraction tooling lives in `conversion_scripts/swf_extraction/`:
...
JSON parsing across `conversion_scripts/` goes through
`conversion_scripts/swf_models.py` (msgspec-backed models + loaders
