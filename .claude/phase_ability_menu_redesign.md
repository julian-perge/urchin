...
**Files:**
- Create: `dev/urchin_dev/swf/extract/ability_icons.py`
...
- [ ] **Step 2: Write `ability_icons.py`**

Copy `dev/urchin_dev/swf/extract/item_icons.py` to `dev/urchin_dev/swf/extract/ability_icons.py` and make exactly these changes:
...
# Run: uv run python3 extract_ability_icons
```
...
```bash
git add dev/swf/ability_icons.py assets/ui/abilities/
...
(Exact expected strings above are copied verbatim from `dev/converted_json/moves_abilities.json`, confirmed via direct inspection before writing this plan - do not
